...
import { ShoppingBag, Search, Minus, Plus, X, ChefHat, Ticket, Info, Bike, Store, MapPin } from 'lucide-react';
...
export default function UserOrder() {
    ...
    // --- PERBAIKAN LOGIKA HARGA (DIKALI 1000) ---
    // Asumsi: Harga di database "10" berarti "10.000"
    const subtotal = cart.reduce((acc, item) => acc + (parseInt(item.price) * 1000 * item.quantity), 0);
    const shippingCost = orderType === 'delivery' ? DELIVERY_FEE : 0;
    ...
    const handleSubmitOrder = async (e) => {
        ...
        if (orderType === 'delivery' && !address) return alert("Mohon isi alamat pengiriman!");
        ...
        try {
            ...
            // Simpan transaksi dengan harga yang SUDAH DIPERBAIKI (x1000) agar laporan benar
            // Kita simpan harga satuan asli (item.price) tetap kecil, tapi total transaksi besar.
            // Atau lebih aman: Kita simpan harga item yang sudah dikali 1000 juga ke database transaksi.
            const fixedCart = cart.map(item => ({
                ...item,
                price: parseInt(item.price) * 1000 // Simpan harga ribuan ke struk
            }));
    ...
    if (orderSuccess) {
        return (
            <div className="min-h-screen bg-[#FDFBF7] flex flex-col items-center justify-center p-6 text-center">
                ...
                <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 mb-6 max-w-xs text-left">
                    <p className="text-blue-800 text-sm font-medium flex gap-2"><Info size={24} className="flex-shrink-0" />{orderType === 'delivery' ? "Mohon tunggu, kurir kami akan segera mengantar pesanan." : "Pesanan akan segera disiapkan. Mohon bersiap ke Kasir."}</p>
                </div>
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 mb-8 w-full max-w-xs relative overflow-hidden"><div className="absolute top-0 left-0 w-full h-2 bg-[#8D6E63]"></div><p className="text-xs font-bold text-gray-400 uppercase mb-2">Nomor Antrian Kamu</p><div className="text-5xl font-black text-[#5D4037] tracking-tighter">{queueNumber}</div><div className="mt-4 flex items-center justify-center gap-2 text-xs text-gray-400 bg-gray-50 py-2 rounded-lg"><Ticket size={14}/> {orderType === 'delivery' ? 'Simpan Nomor Ini' : 'Tunjukkan ke Kasir'}</div></div>
        ...
        <div className="min-h-screen bg-[#FAFAFA] pb-24 max-w-md mx-auto shadow-2xl overflow-hidden relative">
            ...
            {isCartOpen && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center backdrop-blur-sm">
                    <div className="bg-white w-full max-w-md h-[90vh] sm:h-auto sm:rounded-3xl rounded-t-3xl shadow-2xl flex flex-col animate-in slide-in-from-bottom duration-300">
                        <div className="p-5 border-b border-gray-100 flex justify-between items-center"><h2 className="font-bold text-lg text-[#5D4037]">Pesanan Kamu</h2><button onClick={() => setIsCartOpen(false)} className="p-2 bg-gray-100 rounded-full hover:bg-gray-200"><X size={20}/></button></div>
                        <div className="flex-1 overflow-y-auto p-5 space-y-4">
                            {/* HARGA ITEM DI KERANJANG X 1000 */}
                            {cart.map(item => (<div key={item.id} className="flex gap-4 items-center"><img src={item.image} className="w-14 h-14 rounded-lg object-cover bg-gray-50" onError={(e)=>e.target.src='https://placehold.co/100'}/><div className="flex-1"><h4 className="font-bold text-sm text-gray-800">{item.name}</h4><p className="text-[#8D6E63] text-xs font-bold">{formatCurrency(item.price * 1000)}</p></div><div className="flex items-center gap-3 bg-gray-50 rounded-lg p-1"><button onClick={() => updateQty(item.id, -1)} className="w-6 h-6 flex items-center justify-center bg-white rounded shadow-sm text-gray-600"><Minus size={14}/></button><span className="text-xs font-bold w-4 text-center">{item.quantity}</span><button onClick={() => updateQty(item.id, 1)} className="w-6 h-6 flex items-center justify-center bg-[#8D6E63] text-white rounded shadow-sm"><Plus size={14}/></button></div></div>))}
                        </div>
                        <div className="p-6 bg-[#FDFBF7] border-t border-gray-100 rounded-b-3xl space-y-4">
                            <div className="grid grid-cols-2 gap-2 bg-gray-200 p-1 rounded-xl"><button type="button" onClick={() => setOrderType('dine_in')} className={`py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-all ${orderType === 'dine_in' ? 'bg-white text-[#5D4037] shadow-sm' : 'text-gray-500'}`}><Store size={14}/> Ambil Sendiri</button><button type="button" onClick={() => setOrderType('delivery')} className={`py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-all ${orderType === 'delivery' ? 'bg-white text-[#5D4037] shadow-sm' : 'text-gray-500'}`}><Bike size={14}/> Delivery</button></div>
                            <form onSubmit={handleSubmitOrder} className="space-y-4">
                                ...
                                {orderType === 'delivery' && (<div className="animate-in slide-in-from-top duration-300"><label className="block text-xs font-bold text-gray-400 uppercase mb-1 flex items-center gap-1"><MapPin size={12}/> Alamat Pengiriman</label><textarea required placeholder="Alamat Lengkap..." className="w-full p-3 bg-white border border-gray-200 rounded-xl outline-none focus:border-[#8D6E63] text-sm resize-none" rows="2" value={address} onChange={e => setAddress(e.target.value)} /><div className="text-xs text-blue-600 mt-1 font-medium bg-blue-50 p-2 rounded-lg">+ Ongkir Flat Rp {formatCurrency(DELIVERY_FEE)}</div></div>)}
                                <div><label className="block text-xs font-bold text-gray-400 uppercase mb-1">Catatan</label><textarea placeholder="Contoh: Jangan terlalu pedas..." className="w-full p-3 bg-white border border-gray-200 rounded-xl outline-none focus:border-[#8D6E63] text-sm resize-none" rows="1" value={note} onChange={e => setNote(e.target.value)} /></div>
                                <div className="space-y-1 pt-2">
                                    <div className="flex justify-between items-center text-sm text-gray-500"><span>Subtotal</span><span>{formatCurrency(subtotal)}</span></div>
                                    {orderType === 'delivery' && (<div className="flex justify-between items-center text-sm text-blue-600 font-bold"><span>Ongkos Kirim</span><span>{formatCurrency(shippingCost)}</span></div>)}
                                    <div className="flex justify-between items-center text-lg font-bold text-[#5D4037] pt-2 border-t border-dashed border-gray-300 mt-2"><span>Total Bayar</span><span>{formatCurrency(total)}</span></div>
                                </div>
                                <button type="submit" disabled={loading} className="w-full py-4 bg-[#8D6E63] text-white rounded-xl font-bold shadow-lg flex items-center justify-center gap-2 hover:bg-[#5D4037] transition-all">{loading ? 'Mengirim...' : 'Kirim Pesanan Sekarang'}</button>
                            </form>
                        </div>
                    </div>
                </div>
