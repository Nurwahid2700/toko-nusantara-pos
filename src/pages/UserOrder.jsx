...
// Tambahkan getDocs, limit
import { collection, onSnapshot, addDoc, serverTimestamp, query, orderBy, getDocs, limit } from 'firebase/firestore';
import { db } from '../lib/firebase';
import { ShoppingBag, Search, Minus, Plus, X, ArrowRight, ChefHat, Ticket, Info } from 'lucide-react';
...
export default function UserOrder() {
    ...
    // --- LOGIKA NOMOR URUT GLOBAL (SAMA DENGAN POS) ---
    const handleSubmitOrder = async (e) => {
        ...
        try {
            ...
            const qCode = `Q-${nextNumStr}`; // "Q" kode untuk Online
            ...
            <div className="min-h-screen bg-[#FDFBF7] flex flex-col items-center justify-center p-6 text-center">
                ...
                <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 mb-6 max-w-xs text-left">
                    <p className="text-blue-800 text-sm font-medium flex gap-2">
                        ...
                        Pesanan akan segera disiapkan. Mohon untuk bersiap ke <b>Toko Ritel Nusantara</b> dan ambil pesanan Anda.
                    </p>
                </div>

                {/* TIKET ANTRIAN */}
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 mb-8 w-full max-w-xs relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-full h-2 bg-[#8D6E63]"></div>
                    <p className="text-xs font-bold text-gray-400 uppercase mb-2">Nomor Antrian Kamu</p>
                    <div className="text-5xl font-black text-[#5D4037] tracking-tighter">{queueNumber}</div>
                    <div className="mt-4 flex items-center justify-center gap-2 text-xs text-gray-400 bg-gray-50 py-2 rounded-lg">
                        <Ticket size={14}/> Tunjukkan ke Kasir
                    </div>
                </div>

                <button onClick={() => {setOrderSuccess(false); setCustomerName(''); setIsCartOpen(false)}} className="px-8 py-3 bg-[#8D6E63] text-white rounded-xl font-bold shadow-lg">
                    Pesan Lagi
                </button>
            </div>
        );
