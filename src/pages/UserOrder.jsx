...
import { ShoppingBag, Search, Minus, Plus, X, ArrowRight, ChefHat, Ticket, Info, Bike, Store, MapPin } from 'lucide-react';
...
export default function UserOrder() {
    ...
    // STATE BARU: TIPE PESANAN & ALAMAT
    ...
    // Konfigurasi Ongkir Dummy
    const DELIVERY_FEE = 5000; // Ongkir flat Rp 5.000
    ...
    // Jika Delivery, tambah ongkir. Jika tidak, 0.
    ...
    const handleSubmitOrder = async (e) => {
        ...
        try {
            ...
            await addDoc(collection(db, "transactions"), {
                items: cart,
                subtotal: subtotal,   // Harga Barang
                shippingCost: shippingCost, // Ongkir
                total: total,         // Total Bayar
                customerName: customerName,
                note: note,
                
                // DATA PENGIRIMAN BARU
                orderType: orderType, // 'dine_in' atau 'delivery'
    ...
    if (orderSuccess) {
        return (
            <div className="min-h-screen bg-[#FDFBF7] flex flex-col items-center justify-center p-6 text-center">
                ...
                {/* Notifikasi Beda Berdasarkan Tipe Pesanan */}
                <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 mb-6 max-w-xs text-left">
                    <p className="text-blue-800 text-sm font-medium flex gap-2">
                        ...
                        {orderType === 'delivery' 
                            ? "Mohon tunggu, kurir kami akan segera mengantar pesanan ke alamat Anda."
                            : "Pesanan akan segera disiapkan. Mohon bersiap ke Kasir untuk pengambilan."
                        }
                    </p>
                </div>
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center backdrop-blur-sm">
                    <div className="bg-white w-full max-w-md h-[90vh] sm:h-auto sm:rounded-3xl rounded-t-3xl shadow-2xl flex flex-col animate-in slide-in-from-bottom duration-300">
                        ...
                        <div className="p-6 bg-[#FDFBF7] border-t border-gray-100 rounded-b-3xl space-y-4">
                            ...
                            {/* PILIHAN METODE PESANAN (TABS) */}
                            <div className="grid grid-cols-2 gap-2 bg-gray-200 p-1 rounded-xl">
                                <button 
                                    type="button"
                                    onClick={() => setOrderType('dine_in')}
                                    className={`py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-all ${orderType === 'dine_in' ? 'bg-white text-[#5D4037] shadow-sm' : 'text-gray-500'}`}
                                >
                                    <Store size={14}/> Ambil Sendiri
                                </button>
                                <button 
                                    type="button"
                                    onClick={() => setOrderType('delivery')}
                                    className={`py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 transition-all ${orderType === 'delivery' ? 'bg-white text-[#5D4037] shadow-sm' : 'text-gray-500'}`}
                                >
                            ...
                            <form onSubmit={handleSubmitOrder} className="space-y-4">
                                ...
                                {/* JIKA DELIVERY, MUNCUL INPUT ALAMAT */}
                                {orderType === 'delivery' && (
                                    <div className="animate-in slide-in-from-top duration-300">
                                        ...
                                        <textarea required placeholder="Jalan, Nomor Rumah, Patokan..." className="w-full p-3 bg-white border border-gray-200 rounded-xl outline-none focus:border-[#8D6E63] text-sm resize-none" rows="2" value={address} onChange={e => setAddress(e.target.value)} />
                                        <div className="text-xs text-blue-600 mt-1 font-medium bg-blue-50 p-2 rounded-lg">
                                            + Ongkir Flat Rp {formatCurrency(DELIVERY_FEE)}
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="space-y-1 pt-2">
                                    ...
                                    {orderType === 'delivery' && (
                                        <div className="flex justify-between items-center text-sm text-blue-600 font-bold"><span>Ongkos Kirim</span><span>{formatCurrency(shippingCost)}</span></div>
                                    )}
