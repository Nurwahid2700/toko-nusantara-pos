...
// Tambahkan getDocs dan limit untuk cek nomor terakhir
import { collection, onSnapshot, writeBatch, doc, serverTimestamp, query, orderBy, getDocs, limit } from 'firebase/firestore';
import { db } from '../lib/firebase';
import { Search, ShoppingCart, Plus, Minus, Trash2, CreditCard, User, Wallet, QrCode, X, CheckCircle, Ticket } from 'lucide-react';
...
export default function POS() {
    ...
    // --- LOGIKA NOMOR URUT GLOBAL (001, 002, dst) ---
    const getNextQueueNumber = async () => {
        try {
            // Ambil 1 transaksi terakhir berdasarkan waktu pembuatan
            const q = query(collection(db, "transactions"), orderBy("createdAt", "desc"), limit(1));
            const querySnapshot = await getDocs(q);

            let nextNum = 1; // Default jika belum ada transaksi sama sekali

            if (!querySnapshot.empty) {
                const lastTx = querySnapshot.docs[0].data();
                // Format di database misalnya "A-005" atau "Q-005"
                const lastCode = lastTx.queueNumber || "000";
                
                // Ambil angkanya saja (Split berdasarkan "-")
            ...
            // Format jadi 3 digit (contoh: 5 -> "005")
            return String(nextNum).padStart(3, '0');
        } catch (error) {
            console.error("Gagal generate antrian:", error);
            return String(Math.floor(Math.random() * 999)); // Fallback aman
        }
    };
    ...
    const processTransaction = async () => {
        ...
        try {
            // 1. Ambil Nomor Urut Baru
            const numberStr = await getNextQueueNumber();
            const queueCode = `A-${numberStr}`; // "A" kode untuk Admin/Kasir

            setLastQueueNumber(queueCode);
            ...
            batch.set(newTransactionRef, {
                ...
                status: 'pending', 
                queueNumber: queueCode, // Simpan nomor urut
    ...
    return (
        <div className="flex flex-col md:flex-row h-full gap-6 p-6 bg-[#FAFAFA] relative">
            ...
            <div className="w-full md:w-96 bg-white rounded-2xl shadow-xl border border-gray-100 flex flex-col h-[calc(100vh-3rem)] overflow-hidden">
                ...
                <div className="p-5 bg-white border-t border-gray-100 space-y-4"><div><label className="text-xs font-bold text-gray-500 uppercase mb-1 block">Nama Pelanggan</label><input type="text" placeholder="Masukkan nama..." className="w-full pl-4 pr-4 py-2 bg-gray-50 rounded-xl border border-gray-200" value={customerName} onChange={(e) => setCustomerName(e.target.value)} /></div><div className="grid grid-cols-2 gap-2"><button onClick={() => setPaymentMethod('cash')} className={`flex items-center justify-center gap-2 py-2 rounded-xl text-sm font-bold border ${paymentMethod === 'cash' ? 'bg-[#8D6E63] text-white' : 'bg-white'}`}><Wallet size={16} /> Cash</button><button onClick={() => setPaymentMethod('qris')} className={`flex items-center justify-center gap-2 py-2 rounded-xl text-sm font-bold border ${paymentMethod === 'qris' ? 'bg-[#8D6E63] text-white' : 'bg-white'}`}><QrCode size={16} /> QRIS</button></div><div className="pt-2 border-t border-dashed border-gray-200"><div className="flex justify-between items-center mb-4"><span className="text-gray-500 font-medium">Total</span><span className="text-xl font-bold text-[#5D4037]">{formatCurrency(total)}</span></div><button onClick={initiateCheckout} disabled={cart.length === 0 || loading} className="w-full py-3.5 bg-[#8D6E63] text-white rounded-xl font-bold shadow-lg flex items-center justify-center gap-2">{loading ? 'Proses...' : 'Bayar'}</button></div></div>
            </div>
            ...
            {showSuccessModal && (<div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"><div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center animate-in fade-in zoom-in"><div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4"><CheckCircle className="w-10 h-10 text-green-600" /></div><h3 className="text-xl font-bold text-[#5D4037] mb-1">Pembayaran Berhasil!</h3><p className="text-sm text-gray-500 mb-6">Terima kasih Kak {customerName || 'Pelanggan'}</p><div className="bg-[#FDFBF7] p-6 rounded-2xl border-2 border-dashed border-[#8D6E63]/30 mb-6 relative overflow-hidden"><div className="absolute top-0 left-0 w-full h-2 bg-[#8D6E63]/20"></div><p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Nomor Antrian</p><div className="text-5xl font-black text-[#5D4037] tracking-tight">{lastQueueNumber}</div><div className="mt-2 text-[10px] text-gray-400 flex items-center justify-center gap-1"><Ticket size={12} /> Tunjukkan saat dipanggil</div></div><button onClick={() => setShowSuccessModal(false)} className="w-full py-3 bg-[#8D6E63] text-white rounded-xl font-bold shadow-lg">Transaksi Baru</button></div></div>)}
        </div>
    );
}