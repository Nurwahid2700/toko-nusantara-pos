...
import { LayoutDashboard, Package, DollarSign, AlertCircle, Plus, Pencil, Trash2, CheckCircle, Clock, History, ShoppingBag, TrendingUp, QrCode, X, Printer, FileText, Download, Bike, MapPin, Store } from 'lucide-react';
...
export default function Dashboard() {
    ...
    useEffect(() => {
        if (products.length || transactions.length) {
            ...
            // STATISTIK PENDAPATAN (Nominal Murni)
            const revenue = completedTrans.reduce((acc, curr) => acc + (parseInt(curr.total) || 0), 0);
    ...
    const handleExportExcel = () => {
        ...
        const dataToExport = transactions
            ...
            .map(t => ({
                ...
                'Tipe': t.orderType === 'delivery' ? 'Delivery' : 'Dine In',
                'Alamat': t.deliveryAddress || '-',
                                    ...
                                    {products.map((product) => {
                                        ...
                                        return (
                                            <tr key={product.id} className="hover:bg-[#FDFBF7] transition-colors">
                                                ...
                                                {/* HARGA DITAMPILKAN LANGSUNG (Pastikan di DB 15000, bukan 15) */}
                                ...
                                <div className="flex justify-between items-start mb-2">
                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            {/* BADGE STATUS DELIVERY DI DASHBOARD */}
                                            {order.orderType === 'delivery' ? (
                                                ...
                                                <span className="bg-gray-100 text-gray-500 text-[10px] font-bold px-2 py-1 rounded uppercase flex items-center gap-1"><Store size={10}/> Dine In</span>
                                            )}
                                        </div>
                                        <h4 className="font-bold text-[#5D4037]">{order.customerName || 'Pelanggan'}</h4>
                                        {/* TAMPILKAN ALAMAT */}
                                        {order.orderType === 'delivery' && (
                                            <div className="text-[10px] text-gray-600 mt-1 bg-blue-50 border border-blue-100 p-2 rounded flex items-start gap-1">
                                                <MapPin size={12} className="mt-0.5 text-blue-600 flex-shrink-0"/> 
